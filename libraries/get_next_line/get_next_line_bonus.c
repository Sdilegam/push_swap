...
char	*get_next_line(int file_des)
{
	...
	if (!buffer[file_des])
		buffer[file_des] = ft_calloc(1, 1);
	if (!buffer[file_des])
		return (0);
	found = ft_find_chara(buffer[file_des], '\n');
	while (found == -1)
	{
		buffer_text = ft_calloc(BUFFER_SIZE + 2, 1);
		...
		ft_cat(&buffer[file_des], &buffer_text);
	}
	if (!*buffer[file_des])
		return (ft_free(&buffer[file_des]));
	return (ft_return(&buffer[file_des]));
}
 ...
 * 
 * @param string the static variable containing everything that has
 * been read so far.
 * @return char* the next line.
 */
